...
let statusBoxFixed = false;//serve per fissare il box allo scroll
...
function cercaSezioneUrl() {
	...
	for (u=3; u<(MMarrayUrlSkin.length);u++){ 
...
if(SitoHp.indexOf('migliori-notebook') > 0){
	statusSeedTag = false;
	if(larghezzaSitoAdv>970 && MMarrayUrlSkinControllo[1]=='boxfixed'){
        statusBoxFixed = true;
	}
}


//https://www.smartworld.it/migliori-notebook
...
if(document.querySelector('body').getAttribute('data-video') == "1"){
	debugMediamond('[mediamond][inread]===>escluso per variabile editore','worn');
	statusInread = false;
}
	...
	jQuery('<div style="margin: 0 auto; display:block; width: 300px;"><div id="'+divslotpromobox1+'"></div></div>').insertAfter(rifDivslotnameBox);
	...
	jQuery('<div id="container_promobox" class="container_promobox_class"></div>' ).insertAfter(rifDivslotnameBox);
	if(MMlarghezzaSitoAdv > 970) {
		var lipHpsponsor = document.createElement("script");
		lipHpsponsor.async = false;
		lipHpsponsor.type = "text/javascript";
		lipHpsponsor.src = "//adv.mediamond.it/hpsponsor/smartworld/hpsponsor_smartworld.js";
...
//creo un div in cui erogare la tag del out of page
	...
	googletag.cmd.push(function() {
		...
		if(adv_listing == 'listati' || adv_section == 'gallery' || MMstatusRefresh300x250){//i listati sono la home e le home di canale
		...
		googletag.pubads().addEventListener('slotOnload', function(event) {
            ...
            if (document.getElementById(divslotnameBox)) {
				if(event.slot===mm_box1) {
					...
					if(statusBoxFixed)
					bloccoScroll(divslotnameBox);//chiamo la funzione per bloccare lo scroll del box
				}
            }
