...
if(SitoHp.indexOf('black-friday') > 0){
	console.log('[mediamond][adv]===>speciale blackfriday');
	keywordURL += ',black-friday';
}else{
	console.log('[mediamond][adv]===>NO speciale blackfriday');
}
//
...
if(document.querySelector('body').getAttribute('data-video') == "1"){
	...
	//console.log('===> chiata tag gpt in modalitÃ  SRA');
	...
	if(MMlarghezzaSitoAdv > 970) {
		...
		lipHpsponsor.src = "//adv.mediamond.it/hpsponsor/mobileworld/hpsponsor_mobileworld.js";
	...
	googletag.cmd.push(function() {
		...
		if(adv_listing == 'listati' || adv_section == 'mobileworld_gallery' || MMstatusRefresh300x250){//i listati sono la home e le home di canale
		...
		googletag.pubads().addEventListener('slotRenderEnded', function(event) {
			...
			if(event.slot===mm_stript1) {
				...
				if(MMlarghezzaSitoAdv >= 970 && sizeCompletaStrip == '720,240'){
					//mi serve per centrare il minimasthead quando la risoluzione Ã¨ superiore ai 970
					//CenterMinimasthead();
				}
		...
		if(MMlarghezzaSitoAdv > 970 && MMarrayUrlSkinControllo[1] != 'noskin' && adv_section != 'gallery'){
...
window.addEventListener('scroll', function() {
        for (n=2;n<20;n++){
            if( $("#infinite-adslot-"+n).length && statusLoadAdvScroll) {
	...
	if(statusSeedTag){	
		...
		if (cookieSeedtag == ""  || cookieSeedtag == null) {
			if(cookieSeedtag != 'seedtag_'+nomeSito){
				...
				window._seedtagq.push(['_setId', '5196-0553-01']);
